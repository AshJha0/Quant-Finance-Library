...
| `alpha` | Factor research pipeline (signal → IC → validation → cost-aware backtest → construction → report) | `AlphaContext`, `AlphaFactor`, `Factors`, `SignalEvaluator`, `AlphaValidation`, `AlphaBacktester`, `PortfolioConstruction`, `AlphaReport`, `FamaMacBeth` (cross-sectional premia with time-series t-stats — a priced factor vs a dud, tested with planted premia), `CalendarAnomalies` (day-of-week/turn-of-month profiles with the t-stats that keep them honest) | `FactorsTest`, `SignalEvaluatorTest`, `AlphaValidationTest`, `PortfolioConstructionTest`, `AlphaBacktesterReportTest`, `AlphaResearchRoundTest` |
| `microstructure` | Impact, queues, TCA, optimal execution, exchange mechanics, streaming signals + quant models | `MarketImpactModel`, `QueueModel`, `TransactionCostAnalyzer`, `AlmgrenChriss`, `TickSizeSchedule`, `Auction`, `SignalEngine` (unified multi-symbol: imbalance/vol/liquidity/momentum/composite), `FlowSignals` (OFI + imbalances), `CircuitBreakers` (LULD + market-wide); execution-feeding models: `VolumeCurve` (dynamic intraday volume), `VolatilityCurve` (vol seasonality + regime), `QueuePositionEstimator` (L2 queue position), `HiddenLiquidityDetector` (iceberg inference), `SpreadForecaster` (spread prediction), `TradeClassifier` (Lee-Ready), `FillProbabilityModel` (touch × queue); adaptive layer: `OnlineAlphaLearner` (ridge-SGD alpha weights, prequential out-of-sample IC gate), `LeadLagEstimator` (streaming cross-asset lead-lag), `DayTypeProfiles` (per-day-type seasonality curves), `EwmaCovariance` (streaming basket risk matrix: marginal contribution + min-variance hedge ratios), `KylesLambda` (learned impact — the live `MarketState.impactBps` producer), `JumpRobustVolatility` (bipower jump/diffusion separation), `ClosingAuctionModel` (documented-contract auction reserve), `HawkesIntensity` (self-exciting event bursts, stability-enforced), `AlphaEnsemble` (IC-weighted, confidence-sized signal blend), `Vpin` (volume-bucketed flow toxicity), `OrnsteinUhlenbeck` (mean-reversion fit: κ/θ/σ, half-life, stationary z-score — refuses non-reverting series), `LiquidityMeasures` (Roll bounce-implied spread, Corwin-Schultz high-low spread, Amihud illiquidity — liquidity from bars alone), `VarianceRatio` (Lo-MacKinlay trending/reverting/random-walk test + z-stat), `KalmanBeta` (time-varying hedge ratio: tracks the drifting β a static OLS averages away — tested head-to-head) | `MicrostructureTest`, `AlmgrenChrissTest`, `TickSizeAuctionTest`, `SignalEngineTest`, `FlowSignalsTest`, `CircuitBreakersTest`, `QuantModelsTest`, `QuantModels2Test`, `QuantModels3Test`, `QuantModels4Test`, `QuantModels5Test`, `QuantModels6Test`, `QuantSignalsTest`, `AdaptiveQuantAlgosTest`; the full learning loop end-to-end: `integration.OvernightLearningLoopTest` |
...
| `volatility` | Vol models | `EwmaVolatility`, `Garch11`, `GjrGarch11` (leverage-effect asymmetry, MLE; likelihood-beats the symmetric fit on asymmetric data — tested), `Egarch11` (Nelson log-variance: leverage as a sign, no positivity constraints; multi-step forecasts deliberately refused — median ≠ mean), `HarRv` (Corsi daily/weekly/monthly OLS — the realized-vol forecasting benchmark), `VolatilityIndex` (VIX-style model-free fear gauge from an option chain — a flat-vol chain hands its vol back, tested), `VolatilityDecomposition` (systematic vs idiosyncratic: the exact OLS split, cross-checked against `RiskMetrics.beta`) | `VolatilityModelsTest`, `EgarchTest`, `VolatilityIndexAndDecompositionTest`, `MarketRiskTest` (in `risk`), `QuantSignalsTest` (in `microstructure`) |
| `risk` | Portfolio/credit/limit/validation risk + the market-risk workflow ([MARKET_RISK.md](MARKET_RISK.md)) | `RiskMetrics`, `PortfolioRiskAnalyzer`, `VarBacktest`, `PreTradeLimitChecker`, `CounterpartyExposureTracker`, `SettlementRiskAnalyzer`, `ConcentrationRisk`; market risk: `Dependence` (Spearman/Kendall/τ→ρ bridge), `Pca` (Jacobi), `GaussianCopula` (+ Student-t tail dependence), `VarEngine` (delta-normal/MC/delta-gamma/historical/full-revaluation, each with ES), `ExtremeValueTheory` (POT/GPD, refuses infinite-mean ES), `StressTester` (scenarios/ladders/closed-form reverse stress), `FrtbEs` (97.5% ES liquidity-horizon cascade + Basel traffic light — styled after MAR33, not certified), `PnlAttribution` (FRTB PLAT) | `RiskMetricsTest`, `VarBacktestTest`, `CreditAndLimitsTest`, `MarketRiskTest` |
| `hedging` | Hedging algorithms | `DeltaHedger`, `HedgingSimulator`, `GreekHedger`, `OptionsBook`, `MinimumVarianceHedge`, `FxHedger`, `PairsHedger`, `CointegrationTest`, `WhalleyWilmott` (OPTIMAL no-trade band width — cube-root laws pinned exactly; hedge to the nearest edge, never the center) | `HedgingTest`, `HedgingSimulatorTest`, `OptionsBookTest`, `CointegrationTestTest`, `AdaptiveQuantAlgosTest` (in `microstructure`) |
...
| `backtest.validation` | Overfitting defense | `WalkForwardAnalyzer`, `GridSearchOptimizer`, `SharpeValidation` (probabilistic + deflated Sharpe + minimum track record length, round-tripped), `ParameterGrid`, `BlockBootstrap` (stationary bootstrap — the Sharpe's sampling distribution; blocked vs iid width tested head-to-head on autocorrelated returns) | `ValidationTest`, `AlphaResearchRoundTest` (in `alpha`) |
