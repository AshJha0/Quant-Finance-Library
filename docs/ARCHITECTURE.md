...
| `volatility` | Vol models | `EwmaVolatility`, `Garch11`, `GjrGarch11` (leverage-effect asymmetry, MLE; likelihood-beats the symmetric fit on asymmetric data — tested), `HarRv` (Corsi daily/weekly/monthly OLS — the realized-vol forecasting benchmark) | `VolatilityModelsTest`, `MarketRiskTest` (in `risk`), `QuantSignalsTest` (in `microstructure`) |
