...
| `sbe` | Binary flyweight codecs + channel adapters | `TradeFlyweight`, `OrderFlyweight`, `BinaryMarketDataClient`, `BinaryOrderPublisher/Receiver` | `SbeCodecTest` (incl. zero-alloc proof) |
...
| `microstructure` | Impact, queues, TCA, optimal execution | `MarketImpactModel`, `QueueModel`, `TransactionCostAnalyzer`, `AlmgrenChriss` | `MicrostructureTest`, `AlmgrenChrissTest` |
| `pricing` | Fair value & derivatives pricing | `BlackScholes`, `VolSurface`, `SabrModel`, `BinomialTree`, `FairValueEngine`, `TriangularArbitrage`, `ForwardCurve` | `BlackScholesTest`, `VolSurfaceTest`, `AmericanAndSabrTest`, `PricingTest` |
...
| `backtest` | Bar backtesting + execution models | `Backtester`, `ExecutionAwareBacktester`, `Instant/Sor/IcebergExecution` | `BacktesterTest`, `ExecutionAwareBacktesterTest` |
...
| `backtest.tick` | Tick-level backtesting | `TickBacktester` (queue-aware fills, tick-size grid) | `TickBacktesterTest` |
...
| `trading` | Order entry (paper + HFT lane) | `OrderGateway`, `PaperTradingGateway`, `TradingDashboard`; fast lane: `HftRiskGate`, `OrderRingBuffer`, `HftOrderGateway` | `PaperTradingTest`, `TradingDashboardTest`, `HftOrderPathTest` |
...
| `examples` | Runnable demos & benchmarks (coverage-excluded) | `QuickStartDemo`, `HftLatencyBenchmark`, `HftOrderBenchmark` | run manually / bench workflow |
